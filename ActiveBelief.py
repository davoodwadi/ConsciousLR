...
class ActiveBelief(Optimizer):
    ...
    def __init__(self, params, stepSize, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, weight_decouple = False, fixed_decay=False, rectify = False, lrHigh=.05, lrLow=.95 ):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, stepSize=stepSize,
                        lrHigh=lrHigh, lrLow=lrLow,
                        weight_decay=weight_decay, amsgrad=amsgrad)
        super(ActiveBelief, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(ActiveBelief, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if not self.rectify:
                    # Default update
                    step_size = group['lr'] / bias_correction1 * state['gai']
                    ...
                    if state['rho_t'] > 4: # perform Adam style update if variance is small
                        ...
                        step_size = rt * group['lr'] / bias_correction1 * state['gai']

                        p.data -= step_size * (exp_avg / denom)

                    else: # perform SGD style update
                        # p.data.add_( -group['lr']* state['gai'], exp_avg)
                        p.data -= group['lr'] * state['gai'] * exp_avg
                # SetLR if i>0
                if state['step']/group['stepSize'] > 1 and state['step']%group['stepSize']==0:
                    tmp2 = state['gradOld'].clone()##could be eliminated
                    tmp3 = state['cumm'].clone()##could be eliminated
                    tmp5 = state['gai'].clone()##may be the one that needs cloning
